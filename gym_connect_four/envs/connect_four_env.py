...
class ConnectFourEnv(gym.Env):
    ...
    class StepResult(NamedTuple):
        ...
        def get_reward(self):
            ...
            elif self.res_type is ResultType.INVALID:
                return ConnectFourEnv.INVALID_REWARD
            elif self.res_type is ResultType.LOSS:   
                return ConnectFourEnv.LOSS_REWARD
            else:
                return ConnectFourEnv.WIN_REWARD
    ...
    def _step(self, action: int) -> StepResult:
        ...
        if not self.is_valid_action(action):
            return self.StepResult(ResultType.INVALID)
        else:
            # Check and perform action
            for index in list(reversed(range(self.board_shape[0]))):
                if self.__board[index][action] == 0:
                    self.__board[index][action] = 1
                    break

            # Check if board is completely filled
            if np.count_nonzero(self.__board[0]) == self.board_shape[1]:
                return self.StepResult(ResultType.DRAW)
            else:
                # Check win condition
                if self.is_win_state():
                    return self.StepResult(ResultType.WIN)
        
        # Player 2
        action2 = self.agent2.get_next_action(self.__board)
        #print("action1", action,"action2", action2)
        if not self.is_valid_action(action2):
            raise ValueError('player 2 is insane')
        else:
            # Check and perform action
            for index in list(reversed(range(self.board_shape[0]))):
                if self.__board[index][action2] == 0:
                    self.__board[index][action2] = -1
                    break
            ...
            if np.count_nonzero(self.__board[0]) == self.board_shape[1]:
                ...
                # Check win condition
                if self.is_win_state():
                    return self.StepResult(ResultType.LOSS)
                    
        return self.StepResult(ResultType.NONE)
